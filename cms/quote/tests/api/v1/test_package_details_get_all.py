...
class TestPackageDetailsGetAll(TestCase):
    ...
    def test_package_details_get_all(self):

        response = client.get(self.package_details_url)
        ...
        self.assertEqual('Package details are retrieved successfully',
                         api_response['message'])

        data = api_response['data']
        package_details_keys = PackageDetailsRequestParams.package_details_get_all_keys()

        for package in data:
            for package_details_key in package_details_keys:
                self.assertIn(package_details_key, package)

    def test_package_details_get_all_with_pagination(self):

        api_url = PackageDetailsRequestParams.api_url(
        ) + '/api/v1/company/'+str(self.company_id)+'/quote/'+str(self.quote_id)+'/package_details/?page=1&limit=2'
    ...
    def test_package_details_get_all_quote_not_found(self):
    ...
    def test_package_details_get_all_access_denied(self):

        api_url = PackageDetailsRequestParams.api_url(
        ) + '/api/v1/company/9999/quote/'+str(self.quote_id)+'/package_details/'
