...
class TestBranchTransportModeUpdateById(TestCase):
    '''Test modules for deletion of branch (PUT method) '''
    ...
    def test_transport_mode_update_by_id(self):
        ...
        response = client.put(api_url, data=json.dumps(params), content_type='application/json')
        api_response = json.loads(response.content)

        self.assertEqual(200, response.status_code)
        self.assertEqual('success', api_response['status'])
        self.assertEqual(
            'Branch transport mode details are updated successfully', api_response['message'])
