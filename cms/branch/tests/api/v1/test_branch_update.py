...
class TestBranchUpdate(TestCase):
    ...
    def test_branch_update(self):
        ...
        api_url_update = BranchRequestParams.api_url() + 'api/v1/company/' + \
            str(self.company_id)+'/branch/' + \
            str(self.branch_id)+'/'

        params = BranchRequestParams.branch_update(self)
        response = client.put(api_url_update, data=json.dumps(params), content_type='application/json')
        ...
        self.assertEqual('Branch updated successfully',
                         api_response['message'])
        self.assertEqual(None, api_response['data'])

    def test_branch_update_company_not_found(self):

        api_company_not_found_url = BranchRequestParams.api_url(
        ) + 'api/v1/company/99999999999/branch/'+str(self.branch_id)+'/'

        params = BranchRequestParams.branch_update(self)
        response = client.put(api_company_not_found_url, data=json.dumps(params), content_type='application/json')
    ...
    def test_branch_update_details_not_found(self):

        api_details_not_found_url = BranchRequestParams.api_url(
        ) + 'api/v1/company/'+str(self.company_id)+'/branch/999999999/'

        params = BranchRequestParams.branch_update(self)
        response = client.put(api_details_not_found_url, data=json.dumps(params), content_type='application/json')
    ...
    def test_branch_update_name_already_exist(self):
