...
class TestCompanyLogisticInfoUpdateById(APITestCase):
    ...
    def test_company_create(self):
        ...
        # Make a request to update companyLogistic with currency  details by ID
        ...
        params = CompanyLogisticRequestParams.update_company_logistic_Info_with_currency()
        ...
        test_scenario_lable = 'Update companylogisticInfo with currency details in companylogisticInfo module.'
        test_case_lable = 'UPdate companylogisticInfo valid details by id.'
        api_method_lable = 'Update'
        params_lable = 'params'
        ...
        expected_api_message = "Company logistic info updated successfully"
        ...
        # Make a request to update companyLogistic with currency  details by invalid company ID

        company_name = params['name']
        company_id = list(Company.find_by(
            multi=True, name=company_name).values_list('id', flat=True))[0]
        CompanyLogisticInfo_id = list(Company.find_by(
            multi=True, name=company_name).values_list('id', flat=True))[0]
        api_url = CompanyLogisticRequestParams.api_url() + '/api/v1/company/' + \
            str(company_id) + '2/company_logistic_info/' + \
            str(CompanyLogisticInfo_id) + '/'
        response = self.client.put(api_url, data=params, format='json')
        api_response_byte = response.content
        api_response_utf8 = api_response_byte.decode('utf8')
        api_response_json = json.loads(api_response_utf8)

        test_case_lable = 'Update companylogisticInfo valid details by invalid  id.'
